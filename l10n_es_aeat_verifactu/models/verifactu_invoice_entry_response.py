...
class VerifactuInvoiceEntryResponse(models.Model):
    ...
    _inherit = ["mail.message", "mail.thread"]
    _order = "id desc"
    ...
    def _compute_activity_type_id(self):
        for record in self:
            activity = self.env["mail.message"].search(
                [
                    ("model", "=", "verifactu.invoice.entry.response"),
                    ("res_id", "=", record.id),
                ],
                limit=1,
            )
    ...
    def create_activity_on_exception(self):
        ...
        exception_activity_type = self.env.ref(
            "l10n_es_aeat_verifactu.mail_activity_data_exception"
        )
        activity_vals = []
        responsible_group = self.env.ref(
            "l10n_es_aeat_verifactu.group_verifactu_responsible"
        )
        users = responsible_group.users
        for record in self:
            existing = self.env["mail.message"].search_count(
                [
